...
class ProtocolDSlGenerator extends AbstractGenerator {
	...
	override void doGenerate(
		...
		messages.forEach [
			...
			fsa.generateFile(it.name + '.c', generateMessageCSource(it))
	...
	static def rx_st_name(Entry e) '''«e.parent_msg_name.toString.toUpperCase»_RX_READ_«e.name.toUpperCase»'''

	static def t_name(Message msg) '''t_«msg.name»_msg'''

	static def s_name(Message msg) '''s_«msg.name»_msg'''

	static def s_name(BitMask bm) '''s_«bm.parent_msg_name»_«bm.name.toLowerCase»'''

	static def parent_msg(Entry e) { e.eContainer as Message }

	static def parent_msg_name(Entry e) '''«e.parent_msg.name»'''

	static def c_hex(String value) '''0x«value.substring(0,value.length-1)»'''

	static def c_name(EnumSpec e) {
		val msgName = (e.eContainer as Message).name
		'''«msgName»_«e.name.toLowerCase»_enum'''
	}

	static def next(Entry e) { EcoreUtil2.getNextSibling(e) as Entry }
}
